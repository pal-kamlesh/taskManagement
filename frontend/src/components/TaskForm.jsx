...
import { Star, Zap, CheckCircle, Calendar, Pencil, PlusCircle } from 'lucide-react';
...
function TaskForm() {
  ...
  useEffect(() => {
    ...
    // If editing, fetch the task details
    if (isEditing) {
      fetchTask();
    }
    ...
    <div className="max-w-lg mx-auto my-8 p-6 rounded-xl shadow-lg bg-gradient-to-br from-purple-50 to-indigo-50 border-2 border-indigo-200">
      {showConfetti && (
        <div className="fixed top-0 left-0 w-full h-full pointer-events-none z-50">
          ...
          <div className="absolute text-4xl animate-bounce" style={{left: '20%', animationDelay: '0.2s'}}>🎈</div>
          <div className="absolute text-4xl animate-bounce" style={{left: '40%', animationDelay: '0.5s'}}>🎊</div>
          <div className="absolute text-4xl animate-bounce" style={{left: '60%', animationDelay: '0.3s'}}>✨</div>
          <div className="absolute text-4xl animate-bounce" style={{left: '80%', animationDelay: '0.7s'}}>🌟</div>
        </div>
      )}
      
      <div className="flex items-center justify-center mb-6">
        <div className={`text-3xl ${isEditing ? 'text-indigo-600' : 'text-purple-600'} font-bold flex items-center gap-2`}>
          {isEditing ? (
            <><Pencil className="text-indigo-500" size={24} /> Edit Your Adventure!</>
          ) : (
            <><PlusCircle className="text-purple-500" size={24} /> Create New Mission!</>
          )}
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="space-y-2">
          <label htmlFor="title" className="block text-lg font-medium text-indigo-700 flex items-center gap-1">
        ...
        <div className="space-y-2">
          <label htmlFor="description" className="block text-lg font-medium text-indigo-700 flex items-center gap-1">
        ...
        <div className="space-y-2">
          <label htmlFor="status" className="block text-lg font-medium text-indigo-700 flex items-center gap-1">
          ...
          <div className="relative">
            ...
            >
              <option value="To Do" className="bg-white text-red-600">To Do {statusConfig['To Do'].emoji}</option>
              <option value="In Progress" className="bg-white text-blue-600">In Progress {statusConfig['In Progress'].emoji}</option>
              <option value="Done" className="bg-white text-green-600">Done {statusConfig['Done'].emoji}</option>
            </select>
        ...
        <div className="space-y-2">
          <label htmlFor="dueDate" className="block text-lg font-medium text-indigo-700 flex items-center gap-1">
          ...
          <div className="relative">
            <input
              ...
              className="w-full px-4 py-3 rounded-lg border-2 border-indigo-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 text-lg transition-all duration-200 hover:border-indigo-400"
            />
            <Calendar className="absolute right-3 top-1/2 transform -translate-y-1/2 pointer-events-none text-indigo-500" size={20} />
          </div>
        </div>
