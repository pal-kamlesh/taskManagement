...
import { Search, Calendar, Clock, CheckCircle, ArrowUp, ArrowDown, Star, Filter, Trash2, Edit3 } from 'lucide-react';
    ...
    <div className="flex flex-wrap justify-center gap-2 mb-4">
      <button
        ...
        className={`px-4 py-2 rounded-full font-medium transition-all duration-200 transform hover:scale-105 flex items-center gap-1
          ${selectedStatus === 'all' ? 'bg-purple-600 text-white' : 'bg-purple-100 text-purple-800'}`}
      >
      ...
      <button
        ...
        className={`px-4 py-2 rounded-full font-medium transition-all duration-200 transform hover:scale-105 flex items-center gap-1
          ${selectedStatus === 'To Do' ? 'bg-red-500 text-white' : 'bg-red-100 text-red-800'}`}
      >
        <Star size={16} /> To Do 🚀
      </button>
      <button
        ...
        className={`px-4 py-2 rounded-full font-medium transition-all duration-200 transform hover:scale-105 flex items-center gap-1
          ${selectedStatus === 'In Progress' ? 'bg-blue-500 text-white' : 'bg-blue-100 text-blue-800'}`}
      >
      ...
      <button
        ...
        className={`px-4 py-2 rounded-full font-medium transition-all duration-200 transform hover:scale-105 flex items-center gap-1
          ${selectedStatus === 'Done' ? 'bg-green-500 text-white' : 'bg-green-100 text-green-800'}`}
      >
...
function TaskList() {
  ...
  const handleStatusChange = async (task, newStatus) => {
    // If this was real, you would update the task status via API
  ...
  if (loading) return (
    <div className="flex justify-center items-center h-40">
      <div className="text-center">
        ...
        <p className="text-lg font-medium text-indigo-600">Loading your awesome missions...</p>
      </div>
    </div>
  );
        ...
        <div className="flex flex-wrap justify-center gap-3 mt-4">
          <button
            ...
            className={`px-4 py-2 rounded-full font-medium transition transform hover:scale-105 flex items-center gap-1
              ${sortBy === 'dueDate' ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-800'}`}
          >
            <Calendar size={16} />
            Due Date
            {sortBy === 'dueDate' && (
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full font-medium transition transform hover:scale-105 flex items-center gap-1
              ${sortBy === 'title' ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-800'}`}
          >
            A-Z
            {sortBy === 'title' && (
              sortDirection === 'asc' ? <ArrowUp size={16} /> : <ArrowDown size={16} />
          ...
          <button
            ...
            className={`px-4 py-2 rounded-full font-medium transition transform hover:scale-105 flex items-center gap-1
              ${sortBy === 'status' ? 'bg-indigo-600 text-white' : 'bg-indigo-100 text-indigo-800'}`}
          >
            Status
            {sortBy === 'status' && (
              sortDirection === 'asc' ? <ArrowUp size={16} /> : <ArrowDown size={16} />
      ...
      {filteredTasks.length === 0 ? (
        <div className="text-center py-10 bg-indigo-50 rounded-xl border-2 border-dashed border-indigo-200">
          ...
          <p className="text-xl text-gray-600">
            {searchTerm ? "No matching missions found." : "No missions found. Time to add some!"}
          </p>
          {searchTerm && (
            <button 
          ...
          {filteredTasks.map(task => {
            ...
            return (
              ...
              >
                ...
                <div className="p-5">
                  <h3 className={`text-xl font-bold ${colors.text} mb-2`}>{task.title}</h3>
                  ...
                  <div className="flex flex-wrap gap-2 mb-4">
                    ...
                    <span className="inline-block px-3 py-1 rounded-full text-sm font-medium bg-white bg-opacity-50 text-gray-700 flex items-center gap-1">
                      <Calendar size={14} />
